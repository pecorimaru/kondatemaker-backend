...
def fetch_default_sets_by_ingred(
    ingred_nm: str,
    ...
    ingred_form_service = IngredFormService(
        login_info.user_id,
        login_info.group_id,
...
def fetch_ingred_nm_suggestions(
    input_value: str,
    db: Session = Depends(get_db),
    token: str = Depends(oauth2_scheme)
) -> list[str]:
    ...
    ingred_nm_suggestions = ingred_form_service.fetch_ingred_nm_suggestions(
        input_value
    )
