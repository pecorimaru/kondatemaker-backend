...
class WorkCrud(BaseService):
        ...
        try:
            edit_toweek_menu_plan_det = self.db \
                ...
                .filter(
                    ...
                    ToweekMenuPlanDet.owner_user_id == self.owner_user_id,
                ).one()
    ...
    def delete_toweek_menu_plan_det_all(self):
        ...
        try:
            self.db.query(ToweekMenuPlanDet).filter(ToweekMenuPlanDet.owner_user_id == self.owner_user_id).delete()
    ...
    def get_buy_ingred_list(self) -> list[BuyIngred]:
        ...
        try:
            buy_ingred_list = self.db.query(BuyIngred).filter(BuyIngred.owner_user_id == self.owner_user_id).all()
    ...
    def create_buy_ingred_from_selected_plan(self):
        ...
        sql = text(
            ...
            SELECT
                ...
                , MAX(t3.buy_unit_cd)          AS unit_cd
                , MAX(t3.sales_area_type)      AS sales_area_type
                , MAX(c0004.sort_seq)          AS sales_area_seq
            FROM
            ...
            GROUP BY
                ingred_nm
            ORDER BY sales_area_seq
            """
        )

        try:
            result = self.db.execute(sql, {"user_id": self.owner_user_id})
    ...
    def delete_buy_ingred(self, buy_ingred_id: int):
        ...
        try:
            self.db.query(BuyIngred).filter(BuyIngred.buy_ingred_id == buy_ingred_id, BuyIngred.owner_user_id == self.owner_user_id).delete()
