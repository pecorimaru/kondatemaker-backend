...
class IngredCrud(BaseService):
    ...
    def get_ingred(self, ingred_id: int):
        ...
        try:
            ingred = self.db.query(Ingred).filter(Ingred.ingred_id == ingred_id, Ingred.owner_user_id == self.owner_user_id).one_or_none()
            return ingred
    ...
    def get_ingred_from_nm(self, ingred_nm: str) -> Ingred:
        ...
        try:
            ingred = self.db.query(Ingred).filter(Ingred.ingred_nm == ingred_nm, Ingred.owner_user_id == self.owner_user_id).one_or_none()
            return ingred
    ...
    def update_ingred(self, ingred_id: int, ingred_nm: str, ingred_nm_k: str, buy_unit_cd: str, sales_area_type: str):
        ...
        try:
            ...
            edit_ingred.upd_user_id = self.user_id
    ...
    def delete_ingred(self, ingred_id: int):
        ...
        try:
            self.db.query(Ingred).filter(Ingred.ingred_id == ingred_id, Ingred.owner_user_id == self.owner_user_id).delete()
    ...
    def delete_ingred_all(self):

        try:
            self.db.query(Ingred).filter(Ingred.owner_user_id == self.owner_user_id).delete()
            return
    ...
    def get_ingred_unit_conv_from_conv_unit_cd(self, ingred_id: int, conv_unit_cd: str) -> IngredUnitConv:
        ...
        try:
            ingred_unit_conv = self.db \
                ...
                .filter(IngredUnitConv.ingred_id == ingred_id, IngredUnitConv.conv_unit_cd == conv_unit_cd) \
                .one_or_none();
    ...
    def get_ingred_unit_conv_from_conv_unit_cd_list_without_standard(self, ingred_id: int, buy_unit_cd: str) -> list[IngredUnitConv]:
        ...
        try:
            ingred_unit_conv_list = self.db \
                .query(IngredUnitConv) \
                .filter(IngredUnitConv.ingred_id == ingred_id, IngredUnitConv.conv_unit_cd != buy_unit_cd) \
                .all();
    ...
    def create_ingred_unit_conv(self, ingred_id: int, conv_unit_cd: str, conv_rate: float) -> IngredUnitConv:
    ...
    def update_ingred_unit_conv(self, ingred_unit_conv_id: int, ingred_id: int, conv_unit_cd: str, conv_rate: float) -> IngredUnitConv:
    ...
    def replace_ingred_unit_conv(self, ingred_id: int, old_buy_unit_cd: str, new_buy_unit_cd: str):
        ...
        try:
            ...
            if new_standard_ingred_unit_conv:
                ...
                old_standard_ingred_unit_conv.upd_timestamp = time_stamp
                old_standard_ingred_unit_conv.upd_user_id = self.user_id
                old_standard_ingred_unit_conv.version += 1

                # 購入単位(新)の変換レートは 1.0 固定で更新
                new_standard_ingred_unit_conv.conv_rate = 1
                new_standard_ingred_unit_conv.upd_timestamp = time_stamp
                new_standard_ingred_unit_conv.upd_user_id = self.user_id
            ...
            # 購入単位(新)が変換単位として登録されていない場合
                ...
                self.db.query(IngredUnitConv).filter(
                    ...
                    IngredUnitConv.conv_unit_cd == old_buy_unit_cd,
                ).delete()

                # 購入単位(新)の変換レートは 1.0 固定で新規登録
                self.create_ingred_unit_conv(ingred_id, new_buy_unit_cd, 1)
